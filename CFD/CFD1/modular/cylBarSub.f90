program cylBarSub
   ...
   double precision:: nu_, uMean_, dia_, xc_, yc_, chanL_, barL_, barH_
   double precision:: Clen, Crho, Ct, Cnu, CVel, CFor, tau, t
   integer, dimension(0:q - 1)::ex, ey, kb
   double precision:: Cd, Cl, wt(0:q - 1)
   integer::nx, ny, t_, solnumber
   ...
   do t_ = 0, time_
      ...
      if (maxval(rho) .gt. 100.0d0) then
         write (*, *) 'Code Diverged. Automatically Aborted'
         stop
      end if

      call doCollision()
      call doStreaming()
      call applyBC_calcForce()

      if (mod(t_, dispFreq) .eq. 0) then
         write (10, '(I8,4(2X,E12.4))') t_, t, maxval(rho), Cd, Cl
      end if
...
contains
   ...
   subroutine setupDirVec()
      ...
      do a = 0, q - 1
         ...
         if (a .ge. 1 .and. a .le. 4) wt(a) = 4.0d0*one36th
   ...
   end subroutine setupDirVec

   subroutine detectObj()
      implicit none
      integer:: i, j
   ...
   end subroutine detectObj

   subroutine writeSoln(filename_)
      implicit none
      integer:: i, j
      character(len=*), intent(in):: filename_

      open (unit=12, file=filename_)
   ...
   subroutine applyBC_calcForce()
      implicit none
      integer:: i, j, ia, ja, a
      double precision:: uParaRamp_, uPara_, fx_t, fy_t
      double precision:: fx(2) = 0, fy(2) = 0
      !----------------------------------------------------------------------
      ! do i = 2, nx + 1
      !    j = 2
      !    f(5, i, j) = f(8, i, j - 1)
      ...
      fx(1) = fx(2)
      fy(1) = fy(2)
   end subroutine applyBC_calcForce

   subroutine doStreaming()
      implicit none
      integer:: i, j, ia, ja, a
      ...
      do i = 1, nx + 2
         do j = 1, ny + 2
            ...
            do a = 0, q - 1
               tmp1 = uPara_*ex(a)
               tmp2 = uPara_*uPara_
               f(a, i, j) = wt(a)*rhoF_*(1.0d0 + 3.0d0*tmp1 + 4.5d0*tmp1*tmp1 - 1.5d0*tmp2)
            end do
         end do
      end do
   ...
   end subroutine convToLBM

   subroutine allocFlowVars()
      implicit none
