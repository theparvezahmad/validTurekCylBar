program main
  ...
  integer :: nEl, nNodesPerEl, nNodes, nDofPerEl, nDof, nDofBC, coupleRange
  ...
  ! disp(['Static deflection at beam tip(Numerical ):',num2str(DeltaG(nn-3)/L,4)]);
  ! disp(['Static deflection at beam tip(Analytical):',num2str(staticDef_(nNodes)/L,4)]);
  !
  ! figure(1)
  ! hold on
  ! title('Dynamic Nonlinear')
  ! xlabel('Time')
  ! ylabel('ux at Point A')
  ! plot(timeTurekX,XdispTurek,'--k','linewidth',2)
  ! plot(tVec(1601:2001),Xtx(1601:2001) ,'r','linewidth',2)
  ! legend('Benchmark Data','Present Code')
  !
  ! figure(2)
  ! hold on
  ! title('Dynamic Nonlinear')
  ! xlabel('Time')
  ! ylabel('uy at Point A')
  ! plot(timeTurekY,YdispTurek,'--k','linewidth',2)
  ! plot(tVec(1601:2001),Xty(1601:2001) ,'r','linewidth',2)
  ! legend('Benchmark Data','Present Code')

  !
  ! figure(2)
  ! hold on
  ! title('Static Euler Bernoulli')
  ! xlabel('Non-dimensional length')
  ! ylabel('Deflection')
  ! plot(xVec/L',(staticDef_/L)','linewidth',2);
  ! plot(xVec/L',[0;DeltaG(1:2:nn-2)]/L,'k*','linewidth',2);
...
contains
  ...
  subroutine compDynRes(tStart_, tEnd_, dt_, probeDof_)
    ...
    double precision, allocatable, dimension(:, :):: Kg, Mg, MM
    ...
    do while (t .le. tEnd_)
      ...
      do iLoad = 1, noOfLoadSteps
        ...
        !Xi=zeros(nDofBC,1);
        call calcMgKgFg(X, stepLoad, Mg, Kg, Fg)
        !Xi=(a0*Mg + Kg)\(Fg + Mg*(a2*XdOld + a3*XddOld))
        Xi = Fg + mulMatVec(Mg, a2*XdOld + a3*XddOld)
        MM = a0*Mg + Kg
        call DGESV(nDofBC, 1, MM, nDofBC, pivot, Xi, nDofBC, ok)
        ...
        do while (err > tol)
          ...
          Xti = X + Xi
          call calcMgKgFg(Xti, stepLoad, Mg, Kg, Fg)
          !Xii=(a0*Mg + Kg)\(Fg - Mg*Xidd)
          Xii = Fg - mulMatVec(Mg, Xidd)
          MM = a0*Mg + Kg
          call DGESV(nDofBC, 1, MM, nDofBC, pivot, Xii, nDofBC, ok)
          Xi = Xi + Xii
  ...
  subroutine calcMgKgFg(DeltaG, stepLoad, Mg, Kg, Fg)
    implicit none
    double precision, dimension(:), intent(in) :: DeltaG
    double precision, intent(in) :: stepLoad
    double precision, allocatable, dimension(:, :), intent(out) :: Kg, Mg
