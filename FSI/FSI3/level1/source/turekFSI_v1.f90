program turekFSI
   ...
   integer:: i, j, a, t_, ia, ja, kb(0:q - 1), solnumber !, ex(0:q - 1), ey(0:q - 1)
   ...
   integer:: coupleRange, nUdiag, nLdiag, ldab
   ...
   !---------------------------------------------------------
   double precision :: tStart_, tEnd_, dt_
...
!----------------------------------------------------------------------
   ...
   do t_ = 0, time_
      ...
      if (sum(rho)/(nx*ny) .gt. 10.0d0) then
         write (*, *) 'Code Diverged'
         stop
      end if
...
contains
   ...
   subroutine detectDeformedShape(isn)
      ...
      call mapGlobal2Local(DeltaG, DeltaEl)
      isn = 0
      do i = 2, nx + 1
         do j = 2, ny + 2
